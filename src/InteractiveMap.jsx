  ...
  const fetchNationalParksData = async () => {
    try {
      const response = await fetch(
        `https://developer.nps.gov/api/v1/parks?stateCode=UT&api_key=1JhdpsWd0tSFh3X9qx6DhcZk6EefRIjUXlYJUDzY`
      );
      const data = await response.json();

      if (!data || !data.data || data.data.length === 0) {
        console.error(
          "Error fetching national parks data: Data is null or empty"
        );
        return;
      }

      const utahNationallyProtectedAreas = data.data.filter(
        (park) =>
          park.states.includes("UT") &&
          (park.designation.includes("National Park") ||
            park.designation.includes("National Monument") ||
            park.designation.includes("National Recreation Area") ||
            park.designation.includes("National Historic Site") ||
            park.designation.includes("National Historic Park") ||
            park.designation.includes("National Scenic Trail"))
      );
...
useEffect(() => {
  ...
  return (
    <div className="map-container">
      ...
      <InformationBox selectedPark={selectedPark} />{" "}
      {/* Pass the selectedPark to InformationBox */}
    </div>
  );
};
